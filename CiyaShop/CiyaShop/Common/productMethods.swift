...
func getCartUpdatedProduct(product : JSON) -> JSON
{
    if arrCart.count > 0 {
...
func updateProductInCart(product : JSON)
{
    if arrCart.count > 0 {
        
        for i in 0..<arrCart.count
        {
            let cartProduct = arrCart[i]
            if cartProduct["id"].stringValue == product["id"].stringValue
            {
                arrCart[i] = product
            }
...
func addItemInCart(product : JSON) {
    ...
    if(checkItemExistsInCart(product: product) == false) {
        ...
        {
            if(updatedProduct["advanced_qty_step"].exists())
            {
                updatedProduct["qty"].intValue = updatedProduct["qty"].intValue + updatedProduct["advanced_qty_step"].intValue
            }else{
                updatedProduct["qty"].intValue = updatedProduct["advanced_qty_step"].intValue
            }
        }
