...
class CartVC: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            ...
            if(selectedCheckoutType == .buyNow)
            {
                ...
                dictProduct = arrBuyNow[indexPath.row]

            }else{
                cell.setProductData(product: arrCart[indexPath.row])
                dictProduct = arrCart[indexPath.row]
            ...
            cell.lblNotes.text = dictProduct["notes"].stringValue
    ...
    @objc func btnMinusQtyClicked(_ sender: Any) {
        ...
        if(selectedCheckoutType == .cart)
        {
            ...
            let updatedQty = product["qty"].doubleValue - product["advanced_qty_step"].doubleValue
            if product["qty"].doubleValue == product["advanced_qty_step"].doubleValue {
...
//        showAlertWithConfirmation(messageStr: getLocalizationString(key: "RemoveFromCart"), viewController: self) {
//
            ...
            if success {
                if jsonReponse["status"] == "success" {
                    ...
                    for product in arrMyCart {
                        FacebookHelper.logInitiatedCheckoutEvent(contentId: product["id"].stringValue, contentType: product["name"].stringValue, numItems: product["qty"].intValue, paymentInfoAvailable: false, currency: strCurrencySymbol, totalPrice: product["price"].double ?? 0)
                    }
                    
                    // abanonded cart code
                    var strDate = ""
                    do {
                        strDate = try convertDateToString(date: Date(), formatter: "yyyy-MM-dd HH:mm:ss")
                    } catch {}
                    
                    setValueToLocal(key: kFbAbandonedCartTime, value: strDate)
                    storeJSONToLocal(key: kFbAbandonedCartObject, value: arrMyCart.description as Any)
                    //End Facebook pixel
