...
class ProductDetailVC: UIViewController {
    ...
    func registerDatasourceCell()
    {
        ...
        tblProductDetail.register(UINib(nibName: "NotesTableViewCell", bundle: nil), forCellReuseIdentifier: "NotesTableViewCell")
    ...
    func SetUpDetailCells()
    {
        ...
        arrayDetailCells.append(.notesCell)
...
extension ProductDetailVC : UITableViewDelegate,UITableViewDataSource
{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        if(cell == .basicInfoCell)
        {
            ...
            cell.btnPlus.addTarget(self, action: #selector(btnPlusQtyClicked(_:)), for: .touchUpInside)
            cell.btnMinus.addTarget(self, action: #selector(btnMinusQtyClicked(_:)), for: .touchUpInside)
        ...
        else if(cell == .notesCell)
        {
            ...
            cell.handlerNotes = {(txtNotes) in
                
                self.dictProductDetail["notes"].stringValue = txtNotes
                if(checkItemExistsInCart(product: self.dictProductDetail))
                {
                    updateProductInCart(product: self.dictProductDetail)
                }
...
//extension ProductDetailVC : NaviagteToProductDetailDelegate
//{
//    func ShowProductDetail(productDetailDict: JSON)
//    {
//        NavigateToProductDetails(detailDict: productDetailDict)
//    }
//}
//MARK:- Video Player methods
/*extension ProductDetailVC : OpenVideoPlayerDelegate,AVPlayerViewControllerDelegate
{
...
extension ProductDetailVC
{
    ...
    @objc func btnPlusQtyClicked(_ sender: Any) {
        ...
        var product = dictProductDetail
        
//        if(checkItemExistsInCart(product: dictProductDetail))
//        {
//            product = getCartUpdatedProduct(product: product)
//        }
        ...
        {
            updatedQty = product["advanced_qty_step"].doubleValue + product["advanced_qty_step"].doubleValue
        }else{
            updatedQty = product["qty"].doubleValue + product["advanced_qty_step"].doubleValue
        }
        
        if product["manage_stock"].exists()
        {
...
//        }
    ...
    @objc func btnMinusQtyClicked(_ sender: Any) {
        ...
        let product = dictProductDetail
        
        let updatedQty = product["qty"].doubleValue - product["advanced_qty_step"].doubleValue
...
//        }
    ...
    //MARK:-
    func updateQtyAmount()
    {
//        self.tblProductDetail.reloadSections(IndexSet(integer: 0), with: .none)
...
//        availability = selectedVariationJson["in_stock"].boolValue
        ...
        tblProductDetail.reloadData()
//        let offsetY = tblProductDetail.contentOffset.y
//        tblProductDetail.reloadRows(at: [IndexPath(row: 0, section: 0)], with: .none)
//        tblProductDetail.contentOffset.y = offsetY
...
extension ProductDetailVC
{
    func setUpVariationData(jsonResponse:JSON)
    {
       ...
       if(jsonResponse.arrayValue.count > 0) {
        ...
        //TODO:- Facebook Pixel for Product content view
        if (arrayAllVariations.count > 0 && page == 1)
        {
            FacebookHelper.logViewedContentEvent(contentType: dictProductDetail["name"].stringValue, contentId: dictProductDetail["id"].stringValue, currency: strCurrencySymbol, price: dictProductDetail["price"].double ?? 0)
        }
...
//        let strURL = OTHER_API_PATH + "product_deliver_pincode"
        
        /*CiyaShopAPISecurity.postAPICall(strURL, dictParams: params) { (success, message, responseData) in
            let jsonReponse = JSON(responseData as Any)
                       print("jsonReponse - ",jsonReponse)
            
        }*/
        
       /* CiyaShopAPISecurity.checkDelivery(toPincode: params) { (success, message, responseData) in
            ...
            if success {
                 ...
                 if(jsonReponse["status"].stringValue == kSuccess){
                    self.showToast(message: jsonReponse["message"].stringValue + " at " + self.strPincode)
                 }else{
                 self.showToast(message: jsonReponse["message"].stringValue + " at " + self.strPincode)
                 }
                ...
                if let message = jsonReponse["message"].string {
                    ...
                    self.showToast(message: getLocalizationString(key: "technicalIssue"))
                }
            }
            hideLoader()
        ...
        /*CiyaShopAPISecurity.checkDelivery(params) { (success, message, responseData) in
