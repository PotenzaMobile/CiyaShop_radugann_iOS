...
//           self.lblPrice.setPriceForItem(strPrice)
            ...
            //--check
            /*let regularPrice = dictDetails["regular_price"].doubleValue
            let salePrice = dictDetails["sale_price"].doubleValue
            
            if(dictDetails["on_sale"].boolValue)
            {
        ...
        lblAvailability.attributedText = lblAvailability.setUpAvailabilityUI(color: greenColor, strAvailability: "" + getLocalizationString(key: "InStock"),stock:dictDetails["stock_quantity"].intValue)
    ...
    func setUpUI()
    {
        ...
        //---
        lblQuantity.font = UIFont.appBoldFontName(size: fontSize12)
        lblQuantity.textColor = normalTextColor
        
        //---
        btnPlus.backgroundColor = secondaryColor
        btnMinus.backgroundColor = secondaryColor
        
        [btnPlus,btnMinus].forEach { button in
...
//
    ...
    func setUpData()
    {
        ...
        if(checkItemExistsInCart(product: dictDetails) == false)
        {
            if(dictDetails["qty"].exists())
            {
                self.lblQuantity.text = dictDetails["qty"].stringValue

            }else{
                self.lblQuantity.text = dictDetails["advanced_qty_step"].stringValue
            }
        }else{
            let cartProduct = getCartUpdatedProduct(product: dictDetails)
            print("Cart item quantity - ",cartProduct["qty"].stringValue)
            self.lblQuantity.text = cartProduct["qty"].stringValue
        }
        ...
        if priceValue == "" {
           ...
           self.lblPrice.setPriceForItem(dictDetails["price_html"].stringValue)
        }
        */
        SetUpVariationDetails()
        
        //--
//        lblOffer.isHidden = !dictDetails["on_sale"].boolValue
        

        //--
//        let arrayMetaData = dictDetails["meta_data"].arrayValue
        ...
        //---
        /*if dictDetails["on_sale"].exists() && dictDetails["on_sale"].bool == true {
            if dictDetails["sale_price"].exists() && dictDetails["regular_price"].exists() {
                
                if dictDetails["regular_price"].floatValue == dictDetails["sale_price"].floatValue {
                    self.lblOffer.isHidden = true
                } else {
                    let discount = dictDetails["regular_price"].floatValue - dictDetails["sale_price"].floatValue
                    let discountPercentage = discount * 100 / dictDetails["regular_price"].floatValue
                    var strDiscount = ""
